...
class OrderEditWindow(QWidget):
    ...
    def initUI(self):
        ...
        # ===== ЛЕВАЯ КОЛОНКА - ПАРАМЕТРЫ ЗАКАЗА =====
        left_layout = QVBoxLayout()
        left_layout.setSpacing(12)
        
        # Заголовок левой панели
        left_title = QLabel("Параметры заказа")
        left_title.setFont(QFont('Times New Roman', 14, QFont.Bold))
        left_layout.addWidget(left_title)
        
        # Артикул заказа
        article_label_text = QLabel("Артикул заказа:")
        article_label_text.setFont(QFont('Times New Roman', 12, QFont.Bold))
        left_layout.addWidget(article_label_text)
        
        article_h_layout = QHBoxLayout()
        if self.order_id:
            self.article_label = QLabel()
            self.article_label.setStyleSheet("padding: 8px; border: 1px solid #7FFF00; border-radius: 4px;")
            article_h_layout.addWidget(self.article_label)
        else:
            self.article_input = QLineEdit()
            self.article_input.setPlaceholderText("Введите артикул заказа")
            article_h_layout.addWidget(self.article_input, stretch=2)
            generate_btn = QPushButton("Сгенерировать")
            generate_btn.clicked.connect(self.generate_article)
            article_h_layout.addWidget(generate_btn, stretch=1)
        left_layout.addLayout(article_h_layout)
        
        # Клиент
        client_label = QLabel("Клиент:")
        client_label.setFont(QFont('Times New Roman', 12, QFont.Bold))
        ...
        status_label.setFont(QFont('Times New Roman', 12, QFont.Bold))
        ...
        # Пункт выдачи
        pickup_label = QLabel("Пункт выдачи:")
        pickup_label.setFont(QFont('Times New Roman', 12, QFont.Bold))
        left_layout.addWidget(pickup_label)
        self.pickup_combo = QComboBox()
        left_layout.addWidget(self.pickup_combo)
        
        # Даты
        dates_label = QLabel("Дата заказа:")
        dates_label.setFont(QFont('Times New Roman', 12, QFont.Bold))
        ...
        issue_label = QLabel("Дата выдачи:")
        issue_label.setFont(QFont('Times New Roman', 12, QFont.Bold))
        left_layout.addWidget(issue_label)
        self.issue_date = QDateEdit()
        self.issue_date.setDate(QDate.currentDate())
        self.issue_date.setCalendarPopup(True)
        self.issue_date.setSpecialValueText("Не выдано")
        left_layout.addWidget(self.issue_date)
        ...
        right_title = QLabel("Товары в заказе")
        right_title.setFont(QFont('Times New Roman', 14, QFont.Bold))
        right_layout.addWidget(right_title)
        
        # Панель управления товарами
        add_items_layout = QVBoxLayout()
        add_items_layout.setSpacing(8)
        
        product_label = QLabel("Выбрать товар:")
        product_label.setFont(QFont('Times New Roman', 11, QFont.Bold))
        add_items_layout.addWidget(product_label)
        
        self.product_combo = QComboBox()
        add_items_layout.addWidget(self.product_combo)
        
        qty_label = QLabel("Количество:")
        qty_label.setFont(QFont('Times New Roman', 11, QFont.Bold))
        add_items_layout.addWidget(qty_label)
        
        self.quantity_spin = QSpinBox()
        self.quantity_spin.setRange(1, 999)
        self.quantity_spin.setValue(1)
        add_items_layout.addWidget(self.quantity_spin)
        
        add_item_btn = QPushButton("Добавить товар")
        add_item_btn.clicked.connect(self.add_item)
        add_items_layout.addWidget(add_item_btn)
        
        right_layout.addLayout(add_items_layout)
        ...
        total_label_text.setFont(QFont('Times New Roman', 12, QFont.Bold))
        total_layout.addWidget(total_label_text)
        self.total_label = QLabel("0.00 ₽")
        self.total_label.setFont(QFont('Times New Roman', 14, QFont.Bold))
